	...
	<div class="cov-vue-date">
		<div class="datepickbox">
			<input type="text" title="input date" class="cov-datepicker" readonly="readonly" :placeholder="option.placeholder"
			       v-model="date.time" :required="required" @click="showCheck" @foucus="showCheck"
		...
		<div class="datepicker-overlay" v-if="showInfo.check" @click="dismiss($event)"
			...
			<div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
				...
				<div class="button-box">
					<span @click="today">{{option.buttons ? option.buttons.today : 'Today'}}</span>
  ...
  exports.default = {
    ...
    methods: {
        ...
        if (this.limit.length > 0) {
          ...
          try {
            for (var _iterator = this.limit[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
              ...
              if (limit.type == 'fromto') {
                if (limit.from && !limit.to) {
                  show = next.isAfter(limit.from);
                } else if (!limit.from && limit.to) {
                  show = next.isBefore(limit.to);
                } else {
                  show = next.isBetween(limit.from, limit.to);
                }
              }
            }
          } catch (err) {
      ...
      today: function today() {
        var nn = new Date();
        var ctime = nn.getFullYear() + '-' + (nn.getMonth() + 1) + '-' + nn.getDate() + ' 00:00';
        this.checked.currentMoment = (0, _moment2.default)(ctime, 'YYYY-MM-DD HH:mm');
        this.date.time = (0, _moment2.default)(this.checked.currentMoment).format(this.option.format);
        this.showInfo.check = false;
        this.$emit('change', this.date.time);
