...
exports.default = {
    ...
    methods: {
        ...
        nextMonth: function nextMonth(type) {
            var next = null;
            type == 'next' ? next = (0, _moment2.default)(this.checked.currentMoment).add(1,
                'M') : next = (0, _moment2.default)(this.checked.currentMoment).add(-1,
                'M');
            this.showDay(next);
        },
        showDay: function showDay(time) {
            ...
            this.checked.year = (0, _moment2.default)(this.checked.currentMoment).format(
                "YYYY");
            this.checked.month = (0, _moment2.default)(this.checked.currentMoment).format(
                "MM");
            this.checked.day = (0, _moment2.default)(this.checked.currentMoment).format(
                "DD");

            this.displayInfo.month = this.library.month[(0, _moment2.default)(this.checked.currentMoment)
                .month()];
            ...
            for (var i = 1; i <= monthDays; ++i) {
                ...
                if (i == Math.ceil((0, _moment2.default)(currentMoment).format("D")) && (0,
                        _moment2.default)(oldtime, this.option.format).year() == (0,
                        _moment2.default)(currentMoment).year() && (0, _moment2.default)(
                        oldtime, this.option.format).month() == (0, _moment2.default)(
                        currentMoment).month()) {
            ...
            for (var _i = 0; _i < firstDay - 1; _i++) {
                var passiveDay = {
                    ...
                    inMonth: false,
                    action: 'previous',
                    moment: (0, _moment2.default)(currentMoment).date(1).subtract(_i +
                        1, 'days')
                };
            ...
            for (var _i2 = 1; _i2 <= passiveDaysAtFinal; _i2++) {
                var _passiveDay = {
                    ...
                    inMonth: false,
                    action: 'next',
                    moment: (0, _moment2.default)(currentMoment).add(1, 'months').date(
                        _i2)
                };
        ...
        getFilterCondition: function getFilterCondition(limit, day) {
            if (limit.from && !limit.to) {
                return !_moment(this.checked.year + '-' + this.checked.month + '-' + this.pad(
                    day.value)).isAfter(limit.from);
            } else if (!limit.from && limit.to) {
                return !_moment(this.checked.year + '-' + this.checked.month + '-' + this.pad(
                    day.value)).isBefore(limit.to);
            } else {
                return !_moment(this.checked.year + '-' + this.checked.month + '-' + this.pad(
        ...
        setYear: function setYear(year) {
            this.checked.currentMoment = (0, _moment2.default)(year + '-' + this.checked.month +
                '-' + this.checked.day);
        ...
        setMonth: function setMonth(month) {
            ...
            this.checked.currentMoment = (0, _moment2.default)(this.checked.year + '-' + mo +
                '-' + this.checked.day);
            this.showDay(this.checked.currentMoment);
        },
        ...
        setTime: function setTime(type, obj, list) {
            ...
            try {
                for (var _iterator2 = list[Symbol.iterator](), _step2; !(
        ...
        picked: function picked() {
            if (this.option.type === 'day' || this.option.type === 'min') {
                var ctime = this.checked.year + '-' + this.checked.month + '-' + this.checked
                    .day + ' ' + this.checked.hour + ':' + this.checked.min;
                this.checked.currentMoment = (0, _moment2.default)(ctime,
                    "YYYY-MM-DD HH:mm");
                this.time = (0, _moment2.default)(this.checked.currentMoment).format(this.option
                    .format);
            } else {
...
<template>
    <div class="cov-vue-date">
        <div class="datepickbox control">
            <input type="text" title="input date" class="input cov-datepicker" readonly="readonly" placeholder="{{option.placeholder}}"
                v-model="time" :required="required" @click="showCheck" @focus="showCheck" :style="option.inputStyle"
            />
        </div>
            ...
            <div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
                ...
                <div class="cov-date-box" v-if="showInfo.day">
                    <div class="cov-picker-box">
                        ...
                        <div class="day" v-for="day in dayList" track-by="$index" @click="checkDay(day)" :class="{'checked':day.checked,'unavailable':day.unavailable,'passive-day': !(day.inMonth)}">{{day.value}}</div>
                    </div>
                </div>
                    ...
                    <div class="cov-picker-box date-list">
                        <div class="watch-box">
                            <div class="hour-box">
                                ...
                                <ul>
                                    <li class="hour-item" v-for="hitem in hours | orderBy 'value' 1" @click="setTime('hour', hitem, hours)"
                                        :class="{'active':hitem.checked}">{{hitem.value}}</li>
                                </ul>
                            </div>
                            <div class="min-box">
                                <div class="mui-pciker-rule mui-pciker-rule-ft"></div>
                                <div class="min-item" v-for="mitem in mins | orderBy 'value' 1" @click="setTime('min',mitem, mins)" :class="{'active':mitem.checked}">{{mitem.value}}</div>
                            </div>
                        </div>
                    </div>
                </div>
